...
for(ss in 1:30){
  ...
  xsele = masnps.filter[,sam.cor> quantile(sam.cor, 0.6) ]
  p = ncol(xsele)
  # HERRA
  xdat2 = xsele 
  ...
  # ENET
  glmnetcv = cv.glmnet(xsele, y, alpha = 0.1,parallel = T)
...
save(Enet, HERRA,herra.boost,h2aprx,
     file = '/data2/thetm/heritability-LM/outsimulations/screen60.revised.mafull.spa100.h05.rda')
